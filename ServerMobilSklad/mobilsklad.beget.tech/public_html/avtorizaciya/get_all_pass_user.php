...
echo "<td bgcolor='#ffffff'; id='ramka'><a href='./get_all_pass_user.php?id_pa=$row[user_id]'>".$row[user_login]." </a></td>";
echo "</tr>";
...
$pid = $_GET[id_pa];
    ...
    $link = mysqli_connect($host, $user, $password, $database);
    $query = ("UPDATE `users` SET   user_password = '0d8d5cd06832b29560745fe4e1b941cf' WHERE user_id = $pid");
    $result=mysqli_query($link,$query);
    if ($result) {
        // successfully updated

        echo  "<script>alert('Ваш Пароль: user');</script>";
...
<p><a href="./register_user1.php" id='glav'></a></p>

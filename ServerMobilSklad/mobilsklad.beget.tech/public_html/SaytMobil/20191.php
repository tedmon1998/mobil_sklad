...
$query="SELECT name, kod, ves, created_at, updated_at FROM `products` WHERE gorc='mutq' and name_lev='' and YEAR(updated_at) >= YEAR(NOW()) ORDER BY updated_at DESC";
$result=mysqli_query($link,$query);
 if(mysqli_num_rows($result) > 0)
 {
  ...
  {
      echo "<link rel='stylesheet'  href='./style_export.css' media='screen'/>";
